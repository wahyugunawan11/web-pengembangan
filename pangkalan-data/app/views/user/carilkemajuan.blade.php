...
<div class="content">
  ...
  <table class="table table-striped table-bordered table-hover" id="dataTablesexample">
    ...
    <tbody>
      ...
      $i = 1;
      ?>
      @foreach ($laporankemajuan as $result)
      <tr>
        <td>{{$i++}}</td>
        <td>{{$result->judul}}</td>
        ...
        <td><a href="{{URL::to('/viewprogress/'.$result->idlapkemajuan)}}" target="_blank">Lihat</a></td>
      </tr>
      @endforeach
    </tbody>
  </table>
  ...
  <div class="col span_2_of_4">
    <a href="{{URL::to('/progress')}}" class="btn btn-link">Kembali ke pencarian</a>
  </div>
</div>
@stop

...
<div class="content">
  ...
  <table class="table table-striped table-bordered table-hover" id="dataTablesexample">
    ...
    <tbody>
      ...
      $i = 1;
      ?>
      @foreach ($laporankemajuanpkm as $result)
      <tr>
        <td>{{$i++}}</td>
        <td>{{$result->judul}}</td>
        ...
        <td><a href="{{URL::to('/viewpkmprogress/'.$result->idlkemajuanpkm)}}" target="_blank">Lihat</a></td>
      </tr>
      @endforeach
    </tbody>
  </table>
  <?php
    ...
    echo "Laporan kemajuan tidak ditemukan";
  }
  ?>
  <div class="col span_2_of_4">
    <a href="{{URL::to('/pkmprogress')}}" class="btn btn-link">Kembali ke pencarian</a>
  </div>
</div>
@stop

...
function hapus(idbidilmu){
	var hasil = confirm('Mau hapus?');
	if(hasil==true){
		window.location.href="proses_hapus/"+idbidilmu;
	}
}
</script>
@stop
			...
			<table id="example1" class="table table-bordered table-striped">
				...
				<tbody>
					<?php
                        $i = 1;
                    ?>
					@foreach ($bidang as $index => $bidang2)
					<tr>
						<td>{{$i++}}</td>
						<td>{{$bidang2->bidangilmu}}</td>
						<td><a href="{{URL::to('/admin_editfield/'.$bidang2->idbidilmu)}}">Ubah</a> | <button class="btn btn-danger"><a onclick="hapus({{$bidang2->idbidilmu}})">Hapus</button></td>
					</tr>
					@endforeach
				</tbody>
				<tfoot>

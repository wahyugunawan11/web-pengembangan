...
function hapusdosen(iddosen){
	...
	if(hasil==true){
		window.location.href="proses_hapusdosen/"+iddosen;
	}
}
</script>
@stop
			...
			<table id="example1" class="table table-bordered table-striped">
				...
				<tbody>
					...
					<tr>
						...
						<td>{{$dosen2->nidn}}</td>
						<td>{{$dosen2->nama}}</td>
						<td>{{$dosen2->pangkatgolongan}}</td>
						<td>{{$dosen2->username}}</td>
						<td>{{$dosen2->password}}</td>
						<td>{{$dosen2->tempatlahir}}, {{$dosen2->tanggallahir}}</td>
						<td>{{$dosen2->nip}}</td>
						<td>{{$dosen2->fakultas}}</td>
						<td><a href="{{URL::to('/admin_editlecturer/'.$dosen2->iddosen)}}">Ubah</a> | <button class="btn btn-danger"><a onclick="hapusdosen({{$dosen2->iddosen}})">Hapus</button> | <a href="{{URL::to('/admin_detaillecturer/'.$dosen2->iddosen)}}">Lihat</a></td>
					</tr>
					@endforeach
				</tbody>
				<tfoot>

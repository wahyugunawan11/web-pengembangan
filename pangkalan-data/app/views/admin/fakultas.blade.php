...
@section('title')
<title>Fakultas | Administrator | Pangkalan Data Penelitian</title>
@stop
@section('style')
<!-- Bootstrap 3.3.4 -->
...
function hapusfakultas(idfakultas){
	var hasil = confirm('Mau hapus?');
	if(hasil==true){
		window.location.href="proses_hapusfakultas/"+idfakultas;
	}
}
</script>
@stop
...
<ol class="breadcrumb">
    ...
    <li><a href="#">Manajemen fakultas</a></li>
		...
		<div class="box-body">
			<table id="example1" class="table table-bordered table-striped">
				...
				<tbody>
					...
					<tr>
						<td>{{$i++}}</td>
						<td>{{$fakultas2->kode}}</td>
						<td>{{$fakultas2->fakultas}}</td>
						<td><a href="{{URL::to('/admin_editfaculty/'.$fakultas2->idfakultas)}}">Ubah</a> | <button class="btn btn-danger"><a onclick="hapusfakultas({{$fakultas2->idfakultas}})">Hapus</button></td>
					</tr>
					@endforeach
				</tbody>
				<tfoot>

...
function hapusproposal(idproposal){
	var hasil = confirm('Mau hapus?');
	if(hasil==true){
		window.location.href="proses_hapusproposal/"+idproposal;
	}
}
</script>
@stop
...
<li class="treeview active">
	...
	<ul class="treeview-menu">
		<li class="active">
			...
			<ul class="treeview-menu">
				<li class="active"><a href="{{URL::to('/adminproposal')}}"><i class="fa fa-circle-o"></i> Lihat data</a></li>
		...
		<div class="box-body">
			<table id="example1" class="table table-bordered table-striped">
				...
				<tbody>
					...
					<tr>
						<td>{{$i++}}</td>
						<td>{{$proposal2->judul}}</td>
						<td>{{$proposal2->sumberdana}}</td>
						<td>{{$proposal2->tahun}}</td>
						<td>{{$proposal2->read_more}}</td>
						<td>{{$proposal2->programstudi}}</td>
						<td>{{$proposal2->skim}}</td>
						<td>{{$proposal2->bidangilmu}}</td>
						<td>{{$proposal2->fakultas}}</td>
						<td>{{$proposal2->ketua}}</td>
						<td>{{$proposal2->anggota1}}</td>
						<td>{{$proposal2->anggota2}}</td>
						<td>{{$proposal2->anggota3}}</td>
						<td>{{$proposal2->anggota4}}</td>
						<td>{{$proposal2->anggotalain}}</td>
						<td><a href="{{URL::to('/admin_editproposal/'.$proposal2->idproposal)}}">Ubah</a> | <button class="btn btn-danger"><a onclick="hapusproposal({{$proposal2->idproposal}})">Hapus</button></td>
					</tr>
					@endforeach
				</tbody>
				<tfoot>

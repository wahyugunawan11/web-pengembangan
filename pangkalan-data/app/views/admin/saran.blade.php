...
@section('title')
<title>Manajemen Pesan/Saran | Administrator | Pangkalan Data Penelitian</title>
@stop
...
function hapussaran(id_saran){
	...
	if(hasil==true){
		window.location.href="proses_hsaran/"+id_saran;
	}
}
</script>
<script>
			...
			<table id="example1" class="table table-bordered table-striped">
				...
				<tbody>
					<?php
                        $i = 1;
                    ?>
					@foreach ($saran as $index => $saran2)
					<tr>
						<td>{{$i++}}</td>
						<td>{{$saran2->nama}}</td>
						<td>{{$saran2->email}}</td>
						<td>{{$saran2->saran}}</td>
						<td><a href="{{URL::to('/admin_editmessage/'.$saran2->id_saran)}}">Ubah</a> | <button class="btn btn-danger"><a onclick="hapussaran({{$saran2->id_saran}})">Hapus</button></td>
					</tr>
					@endforeach
				</tbody>
				<tfoot>

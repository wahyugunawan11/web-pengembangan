...
@section('title')
<title>Program Studi | Administrator | Pangkalan Data Penelitian</title>
@stop
@section('style')
<!-- Bootstrap 3.3.4 -->
...
function hapusprogramstudi(idprodi){
	var hasil = confirm('Mau hapus?');
	if(hasil==true){
		window.location.href="proses_hapusprogram/"+idprodi;
	}
}
</script>
@stop
...
<ol class="breadcrumb">
    ...
    <li><a href="#">Manajemen program studi</a></li>
		...
		<div class="box-body">
			<table id="example1" class="table table-bordered table-striped">
				...
				<tbody>
					...
					<tr>
						<td>{{$i++}}</td>
						<td>{{$programstudi2->programstudi}}</td>
						<td>{{$programstudi2->fakultas}}</td>
						<td><a href="{{URL::to('/admin_editprogram/'.$programstudi2->idprodi)}}">Ubah</a> | <button class="btn btn-danger"><a onclick="hapusprogramstudi({{$programstudi2->idprodi}})">Hapus</button></td>
					</tr>
					@endforeach
				</tbody>
				<tfoot>
